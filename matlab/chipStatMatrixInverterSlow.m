function f=chipStatMatrixInverterSlow(Sigma, gamma, beta, x, npts);
%CHIPMATRIXINVERTER inverts block tridiagonal matrices for posterior
%chipChip estimation
lambda=x'*Sigma*x;
nTrans=size(x,1);
...
LcoeffXYT(1)=factor*(1-factor^2)^-1*beta^2/(UcoeffInvSigma(1)* ...
                                             (UcoeffInvSigma(1)+beta^2*lambda));
...
UcoeffXXT(end)=beta^2+factor*(1-factor^2)^-1*LcoeffXYT(end);
