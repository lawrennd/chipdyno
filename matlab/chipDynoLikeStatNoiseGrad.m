...
% CHIPDYNO toolbox
% chipDynoLikeStatNoiseGrad.m version 1.4
% FORMAT f=chipDynoLikeStatNoiseGrad(params,data,precs,X,nEffectGenes,R,C);
...
for i=1:nGenes
  ...
  preGradBeta(i)=gradientSigma*(-2*(beta^2+precs(i,npts-1))^-2*beta-alpha(1)^-2*gradAlphaBeta(1)/factor^2);
