function [tf,tfErrors]=chipDynoExpectationsSlow(data,X,Sigma,beta,gamma,mu, ...
...
Z=invSigma*mu';
factor=cos(gamma)^2;
Means=[beta^2*data(1)*x+(1+factor)^-1*Z];
for i=2:npts-1
    Means=[Means;beta^2*data(i)*x+(1-factor)*(1+factor)^-1*Z];
end
Means=[Means;beta^2*data(end)*x+(1+factor)^-1*Z];
m=postCov*Means;
tf=zeros(1,npts);
for i=1:npts
